...
//FSRCNN need to be SHR
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale);
//void FSRCNN(SHR *img_hr, SHR *img_lr, int rows, int cols, int scale);
...
//void double_2_uint8(SHR *double_img, unsigned char *uint8_img, int cols, int rows);
...
int main(int argc, char *argv[])
{
	char *inFile = argv[1];
	char *outFile = argv[2];
	std::string address(argv[3]),port(argv[4]),role(argv[5]),path(argv[6]);
	...
	for (int fcnt = 0; fcnt < num; fcnt++)
	{
		...
		//client read ,then send the read data to server by ABY
		int i, j;
		//SHR array declare not certify (and the memory for array?)
		std::array<SHR, 25344> inP_array;
		double normal;
		for (i = 0; i<inRows; i++)
		for (j = 0; j<inCols; j++)
		{
