...
int main(int argc, char **argv)
{
	...
	SHR *s_input = (SHR *)malloc(inCols * scale * inRows * scale *sizeof(SHR));
	double *img = (double *)malloc(inCols * scale * inRows * scale *sizeof(double));
	double *img_p = img;

	SHR *s_inP_tmp = s_input;
	for (int i = 0; i < inRows * scale * inCols * scale; i++)
	{
		double x;
		fscanf(inFp, "%lf", &x);
		s_inP_tmp[i] = SHR((uint64_t *)&x, bc, party);
        img_p[i] = s_inP_tmp[i].check();
		cout <<img_p[i]<<endl;
	}
