...
int main(int argc, char **argv)
{
	...
	unsigned char *inBuf = (unsigned char *)malloc(inCols *scale * inRows *scale * sizeof(unsigned char));
	...
	// SHR start
	//double *inBuf_tmp = (double *)malloc(inCols *scale * inRows *scale * sizeof(double));
	SHR *s_y = (SHR *)malloc(inCols * scale * inRows * scale * sizeof(SHR));
		...
		for (i = 0; i < inRows*scale; i++)
		{
			for (j = 0; j < inCols*scale; j++)
			{
				...
				if (role == SERVER)
				{
					double x;
					fscanf(inFp, "%lf", &x);
					shared = new uint64_t(*(uint64_t*)&x);
				}
				else
				{
					shared = NULL;
