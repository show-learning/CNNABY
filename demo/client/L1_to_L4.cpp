...
int main(int argc, char *argv[]){
	...
	for (i = 0; i < inRows; i++)
		{
			for (j = 0; j < inCols; j++)
			{
				...
				if(role==CLIENT){
					tmp[0]= *inP++;
					x[cnt]=double(tmp[cnt] / 255.0);
					shared=new uint64_t(*(uint64_t*)&x[cnt]);
				}
				else{
					*inP++;
					shared=NULL;
				...
				*(s_inP_tmp+cnt) = SHR(shared, bc, party, role);
				delete shared;
				*(inP_tmp + cnt) = s_inP_tmp[cnt].check();
			}
		}
	int num_layers = 8;
	/////////// Convolution1 -------- Layer1
	...
	double weights_layer1_tmp[1400];
	SHR s_weights_layer1[1400];
	for (int i = 0; i < 1400; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer1_ptr, "%lf", &weights_layer1_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer1_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer1+i)= SHR(shared, bc, party, role);
		weights_layer1[i]=s_weights_layer1[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer1_tmp[56];
	SHR s_biases_layer1[56];
	for (int i = 0; i < 56; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer1_ptr, "%lf", &biases_layer1_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer1_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer1+i) = SHR(shared, bc, party, role);
		biases_layer1[i]=s_biases_layer1[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	for (int i = 0; i < num_filters; i++)
	{
		...
		imfilter(inP_tmp, kernel, img_fltr_p1, rows, cols, padsize);
	...
	double weights_layer2_tmp[672];
	SHR s_weights_layer2[672];
	for (int i = 0; i < 672; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer2_ptr, "%lf", &weights_layer2_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer2_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer2+i)= SHR(shared, bc, party, role);
		weights_layer2[i]=s_weights_layer2[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer2_tmp[12];
	SHR s_biases_layer2[12];
	for (int i = 0; i < 12; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer2_ptr, "%lf", &biases_layer2_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer2_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer2+i) = SHR(shared, bc, party, role);
		biases_layer2[i]=s_biases_layer2[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double weights_layer3_tmp[1296];
	SHR s_weights_layer3[1296];
	for (int i = 0; i < 1296; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer3_ptr, "%lf", &weights_layer3_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer3_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer3+i)= SHR(shared, bc, party, role);
		weights_layer3[i]=s_weights_layer3[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer3_tmp[12];
	SHR s_biases_layer3[12];
	for (int i = 0; i < 12; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer3_ptr, "%lf", &biases_layer3_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer3_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer3+i) = SHR(shared, bc, party, role);
		biases_layer3[i]=s_biases_layer3[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double weights_layer4_tmp[1296];
	SHR s_weights_layer4[1296];
	for (int i = 0; i < 1296; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer4_ptr, "%lf", &weights_layer4_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer4_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer4+i)= SHR(shared, bc, party, role);
		weights_layer4[i]=s_weights_layer4[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer4_tmp[12];
	SHR s_biases_layer4[12];
	for (int i = 0; i < 12; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer4_ptr, "%lf", &biases_layer4_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer4_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer4+i) = SHR(shared, bc, party, role);
		biases_layer4[i]=s_biases_layer4[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	for(int i=0;i<rows * cols * num_filters4;i++){
		...
		*(s_img_fltr_4+i)= SHR(shared, bc, party, role);
		delete shared;
		cout<< s_img_fltr_4[i].get_d()<<endl;
	}
	free(s_img_fltr_4);
	free(img_fltr_3);
	img_fltr_3 = NULL;
