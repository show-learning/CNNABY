...
int main(int argc,char** argv){
	...
	double weights_layer6_tmp[1296];
	SHR s_weights_layer6[1296];
	for (int i = 0; i < 1296; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer6_ptr, "%lf", &weights_layer6_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer6_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer6+i)= SHR(shared, bc, party, role);
		weights_layer6[i]=s_weights_layer6[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer6_tmp[12];
	SHR s_biases_layer6[12];
	for (int i = 0; i < 12; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer6_ptr, "%lf", &biases_layer6_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer6_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer6+i)= SHR(shared, bc, party, role);
		biases_layer6[i]=s_biases_layer6[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double weights_layer7_tmp[672];
	SHR s_weights_layer7[672];
	for (int i = 0; i < 672; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer7_ptr, "%lf", &weights_layer7_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer7_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer7+i)= SHR(shared, bc, party, role);
		weights_layer7[i]=s_weights_layer7[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double biases_layer7_tmp[56];
	SHR s_biases_layer7[56];
	for (int i = 0; i < 56; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(biases_layer7_ptr, "%lf", &biases_layer7_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&biases_layer7_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_biases_layer7+i)= SHR(shared, bc, party, role);
		biases_layer7[i]=s_biases_layer7[i].check();
		delete shared;
	}
	if(role==SERVER){
	...
	double weights_layer8_tmp[4536];
	SHR s_weights_layer8[4536];
	for (int i = 0; i < 4536; i++)
	{
		uint64_t *shared;
		if(role==SERVER){
			fscanf(weights_layer8_ptr, "%lf", &weights_layer8_tmp[i]);
			shared=new uint64_t(*(uint64_t*)&weights_layer8_tmp[i]);
		}
		else{
			shared=NULL;
		}
		*(s_weights_layer8+i)= SHR(shared, bc, party, role);
		weights_layer8[i]=s_weights_layer8[i].check();
		delete shared;
	}
	if(role==SERVER){
